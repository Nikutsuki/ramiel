    ...
    if (state.preview_media) |media| {
        ...
        if (media.is_video) {
            if (state.video_playbacks.getEntry(media.url_or_id)) |entry| {
                ...
                if (media.is_gif) {
                    media_node = try comp.animatedMedia(video_state.playback, .{ .style = .{
                        .width = .Full,
                        .height = .Full,
                        .object_fit = .contain,
                    } }, .{});
                    ...
                    media_node = try comp.videoPlayer(.{ .base_id = base_id, .font = font_data, .style = .{
                        .width = .Full,
                        .height = .Full,
                        .object_fit = .contain,
                    } }, .{
            ...
            media_node = try ui.asyncImage(.{
                .source = media.url_or_id,
                .intrinsic_size = .{ @floatFromInt(media.width), @floatFromInt(media.height) },
                .style = .{
                    .width = .Full,
                    .height = .Full,
                    .object_fit = .contain,
        ...
        if (media_node) |mn| {
            ...
            const preview_overlay = try ui.div(.{
                .style = .{
                    .position = .absolute,
                    .width = .Full,
                    .height = .Full,
