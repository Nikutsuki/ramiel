...
pub fn build(ui: *AppUIContext, state: *const AppState) anyerror!*AppNode {
    ...
    const handle = try ui.ux().div(.{
        .style = tw.style(.{
            ...
            tw.bg_value(lib.layout.Color.from(.{ 0.30, 0.70, 1.0, 1.0 })),
    ...
    return ui.ux().div(.{
        .style = tw.style(.{
            ...
            tw.items_center,
            tw.gap_px(16.0),
            tw.bg_value(lib.layout.Color.from(.{ 0.08, 0.09, 0.2, 1.0 })),
        }),
        .children = &.{
            try ui.ux().text(.{
                .content = "pointer capture demo",
                .font = font,
                .style = tw.style(.{tw.text_color_value(lib.layout.Color.from(.{ 0.92, 0.95, 1.0, 1.0 }))}),
            }),
            try ui.ux().text(.{
                .content = value_text,
                .font = font,
                .style = tw.style(.{tw.text_color_value(lib.layout.Color.from(.{ 0.70, 0.76, 0.88, 1.0 }))}),
            }),
            track,
        },
    });
